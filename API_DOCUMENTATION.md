...
- **Request Parameters**:
  ...
  - `bankCode` (Optional): Mã phương thức thanh toán muốn hướng người dùng tới trực tiếp.
    ...
    - `NCB`: Mở trực tiếp ngân hàng NCB trong môi trường test.
    - `INTCARD`: Mở trực tiếp trang thanh toán bằng Thẻ thanh toán quốc tế (Visa, Mastercard, v.v.).
    - *Nếu không truyền hoặc để trống*: Người dùng sẽ được đưa tới trang chọn cổng VNPay mặc định hiển thị các phương thức merchant sandbox đang hỗ trợ.
    - Không dùng `VNPAYQR` với merchant sandbox hiện tại vì VNPay có thể trả lỗi "Ngân hàng thanh toán không được hỗ trợ".
- **Response**:
